...
// Manager presents heterogeneous Element data. Each element could represent
...
//
// Push updates occur when the data source changes outside the list. The
...
// the managed list.
//
// Elements that sort outside the view will be ignored. In that case the
...
func (m *Manager) Layout(gtx layout.Context, index int) layout.Dimensions {
	...
	// If the end of the list is visible, try to load history afterward.
	...
	// being visible.
	//
	// The minimum number of elements required to overcome this check is equal to
